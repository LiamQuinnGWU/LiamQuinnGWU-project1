...
public abstract class Creature{
    ...
    public GridPoint getGridPoint(){
        return new GridPoint(point); //return a copy to preseve encapsulation
    }
    ...
    public int dist(Creature c){ return point.dist(c.getGridPoint());}
    //make a random turn
    public void randomTurn() { this.dir = rand.nextInt(4);}

    
    //TODO: add the methods below to the appropriate class(es)
    public void step() {
        // Calculate the new position after taking a step
        int newX = getX() + dirX[getDir()] * stepLen;
        int newY = getY() + dirY[getDir()] * stepLen;
    
        // Wrap around the board if the new position goes off the edge
        if (newX < 0) newX = City.WIDTH - 1;
        else if (newX >= City.WIDTH) newX = 0;
        
    
        if (newY < 0) newY = City.HEIGHT - 1;
        else if (newY >= City.HEIGHT) newY = 0;
